...
contract AlphaSlav is ERC20 {
    ...
    mapping(address => bool) public tokenClaimedByAddress;
    mapping(address => bool) public tokenDoubledByAddress;
    mapping(address => uint256) public delta;
    mapping(address => uint256) public omicron;
    mapping(address => uint256) public omega;


    constructor() ERC20("Alpha Slav", "aSLAV"){
    }

    //202200 per claim. Once per address.
    function claimAlphaSlav() external {
        // Checks
        require(block.timestamp <= _airdropEnd, "Airdrop lasted till June 29th");
        require(!tokenClaimedByAddress[_msgSender()], "Already claimed");

        // Effects
        tokenClaimedByAddress[_msgSender()] = true;

        // Interactions

        // Minting the coin for the _msgSender
        _mint(_msgSender(), 202200 * (10 ** decimals()));
    }
    ...
    function donate() external payable {
        ...
        _mint(_msgSender(), 2022000 * msg.value);
    }

    //23-24.08.2022
    //Day of the National Flag of Ukraine and Independence day of Ukraine
    //Double all αSLAV on your account
    function doubleAllAlphaSlav() external {
        // Checks
        require(block.timestamp >= _independenceDay - 86400 && block.timestamp <= _independenceDay + 86400, "Wrong day");
        require(!tokenDoubledByAddress[_msgSender()], "Already doubled");

        // Effects
        tokenDoubledByAddress[_msgSender()] = true;
